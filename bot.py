...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        # Track this group so broadcasts can reach it
        ...
        if not valid_drop_message:
            ...
            await context.bot.send_message(
                chat_id,
                f"{handle} this is a link-drop session, not your notes app. "
        ...
        if existing:
            ...
            await context.bot.send_message(
                chat_id,
                f"{handle} really thought nobody would notice. "
                f"{original_poster} already posted that link. Try a different one.",
                **thread_kwargs,
            )
            return
        ...
        if not can_post:
            ...
            await context.bot.send_message(
                chat_id,
                f"{handle} relax. You just posted. "
                f"{remaining} more people need to drop before it's your turn again. "
                f"Progress: {count_after}/{queue_size}. Sit tight.",
                **thread_kwargs,
            )
            return
